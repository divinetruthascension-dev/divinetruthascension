/data/data/com.termux/files/usr/bin/bash
 ...
 --- 1. Variables ---
REPO_URL=https://github.com/settings/keys:divinetruthascension-dev/DivineTruthAscension.git"  # Update
BRANCH= main
PROJECT_DIR=$HOME/divinetruthascension
LOG_DIR=$PROJECT_DIR/npm-logs
SSH_KEY=$HOME/.ssh/id_rsa.pub
GITHUB_TOKEN=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  # Must have repo + admin:public_key
...
if ! command -v netlify >/dev/null 2>&1; then
 ...
 --- 5. SSH Key Setup ---
if [  -f $SSH_KEY ]; then
  echo 🔑 null Generating SSH key...
null   ssh-keygen -t rsa -b 4096 -f $HOME/.ssh/id_rsa -N 
else
...
API_RESPONSE=$(curl -s -X POST -H Authorization: token $GITHUB_TOKEN \
  ...
  -d {\title\: \Termux-DTA-$(date +%s)\, \key\: \$PUB_KEY\})
